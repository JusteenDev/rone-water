          ...
          {aboutData && aboutData.length > 0 ? (
            aboutData.map((about) => (
              <div key={about.id} className="card bg-base-100 shadow-lg p-4 flex flex-col justify-between rounded-lg">
                <img src={about.PHOTO_URL} alt="About section" className="w-full object-cover rounded-md mb-4" />
                ...
                <div className="flex gap-2 mt-4 items-center place-content-center">
                  <button onClick={() => deleteAbout(about.id, about.PHOTO_URL)} className="btn btn-sm btn-error w-24">Delete</button>
                  <button onClick={() => handleEditClick(about)} className="btn btn-sm btn-info w-24">Edit</button>
                </div>
              </div>
            ))
          ) : (
